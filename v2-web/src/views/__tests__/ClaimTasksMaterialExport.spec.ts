...
describe('ClaimTasksView material export integration', () => {
  ...
  it('renders the task cards before export summaries finish loading', async () => {
    ...
    expect(wrapper.find('[data-testid="material-export-toolbar"]').exists()).toBe(true)
  ...
  it('shows compact controls only to administrators and sorts priority by progress', async () => {
    ...
    expect(admin.get('[data-testid="material-export-batch"]').text()).toContain('一键导出已勾选终端')
  ...
  it('disables export only for a completely unconstructed terminal', async () => {
    ...
    expect(zero.get('input[type="checkbox"]').attributes('disabled')).toBeDefined()
    expect(partial.get('input[type="checkbox"]').attributes('disabled')).toBeUndefined()
    wrapper.unmount()
  })
})
