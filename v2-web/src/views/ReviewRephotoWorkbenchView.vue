  ...
  <main class="review-rephoto-workbench" :aria-busy="loading || mutationPending">
    ...
    <template v-if="opened">
      ...
      <section class="workbench-grid">
        ...
        <aside class="review-actions"><h2>审阅与操作</h2><p v-for="reason in terminalBlockers" :key="reason" class="lock-reason">{{ reason }}</p><button type="button" class="rephoto-mutation" data-testid="refresh-terminal" :disabled="!mutable || !sourceChanged" @click="refresh">刷新</button><button type="button" class="rephoto-mutation" data-testid="replace-all-missing" :disabled="!mutable || opened.workflow_state === 'pool_shortage' || !(rephoto?.pool_summary.required)" @click="replaceMissing">随机替换全部缺失采集器</button></aside>
      </section>
      <section v-if="rephoto" class="collector-rail"><h2>采集器</h2><article v-for="row in deduplicatedCollectors" :key="row.original_collector_no" class="collector-card"><strong>{{ row.original_collector_no }}</strong><span>{{ row.physical_state === 'present' ? '有实物' : row.physical_state === 'missing' ? '无实物' : '已替换' }}</span><span v-if="row.final_collector_no && row.final_collector_no !== row.original_collector_no">{{ row.final_collector_no }}</span><Code128Barcode v-if="row.collector_barcode" :value="row.collector_barcode" /><button v-if="row.workbench_item_id" type="button" class="rephoto-mutation" :data-testid="`complete-collector-${row.requirement_id}`" :disabled="!mutable || opened.workflow_state === 'pool_shortage' || completionBlockers(row).length > 0" @click="completeCollector(row)">完成</button><button v-if="row.assignment_id" type="button" class="rephoto-mutation" data-testid="rollback-assignment" :disabled="!mutable" @click="rollback(row)">回滚</button></article></section>
    </template>
  </main>
</template>
