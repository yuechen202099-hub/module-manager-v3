...
const isGroupDialog = computed(() => props.row?.kind === 'group')
...
async function loadDetail() {
  if (!props.modelValue || !props.row || props.row.kind !== 'group') return
  ...
  errorMessage.value = ''
  try {
    const next = await fetchDataCenterDetail('group', props.row.id, detailAbortController.signal)
    if (current !== detailSerial || !props.modelValue || props.row?.id !== next.id) return
    applyDetail(next)
    void loadPhotoObjectUrls(next, current)
  } catch (error) {
    if (current !== detailSerial || (error instanceof DOMException && error.name === 'AbortError')) return
...
watch(
  ...
  ([open, kind]) => {
    cleanupDetail()
    if (open && kind === 'group') void loadDetail()
  },
  { immediate: true },
)
</script>
...
<template>
  ...
  <el-dialog
    v-else
    :model-value="props.modelValue && isGroupDialog"
    ...
    @update:model-value="emit('update:modelValue', $event)"
    @closed="cleanupDetail"
  >
    <template #header>
      <div class="review-title">
        <strong>{{ detail?.meterNo || props.row?.meterNo || props.row?.id }}</strong>
        <span>{{ detail?.terminal || props.row?.terminal }}</span>
      </div>
    </template>
      ...
      <section class="review-pane">
        <el-form label-position="top" class="field-grid">
          ...
          <el-form-item label="模块">
            <el-input v-model="form.moduleAssetNo" />
          </el-form-item>
          <el-form-item label="采集器">
            <el-input v-model="form.collector" />
          </el-form-item>
          <el-form-item label="原因" class="wide-field">
...
.photo-stage :deep(.review-image-inspector) {
  width: 100%;
  height: 100%;
  min-height: 520px;
...
.photo-stage :deep(.review-image-inspector__stage) {
  min-height: 0;
}
