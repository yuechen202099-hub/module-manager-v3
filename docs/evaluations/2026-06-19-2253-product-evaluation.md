...
| 检查项                                                                                                                                 | 结果     | 说明                                                                                                                                                                                                                           |
| ----------------------------------------------------------------------------------------------------------------------------------- | ------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Vue 页面登记                                                                                                                            | 通过     | native=2，legacy=5；legacy 页面：project-board, construction, construction-cache, unmatched, sync-config                                                                                                                          |
...
| C:\Users\Administrator\Desktop\2025模块改造\模块更换项目管理器V2.0\.venv\Scripts\python.exe scripts/verify_vue_migration_gate.py                 | 通过     | [OK] Vue shell and static page registry are wired. / [INFO] registered pages: 7 / [INFO] legacy bridge pages: 5 / [WARN] strict native migration is not complete: /   - construction: legacy_bridge /   - construction-cache |
| C:\Users\Administrator\Desktop\2025模块改造\模块更换项目管理器V2.0\.venv\Scripts\python.exe scripts/verify_vue_migration_gate.py --strict-native | 未通过(1) | [FAIL] Pages still running through legacy bridge: construction, construction-cache, project-board, sync-config, unmatched                                                                                                    |

## 自动扣分上限

- 存在 legacy bridge 页面，前端结构最高按 72 分计，不能以视觉完成度抵消结构风险。
...
- Vue 原生生产入口：未通过，仍有 legacy 页面：project-board, construction, construction-cache, unmatched, sync-config。
- `python scripts/verify_vue_migration_gate.py --strict-native`：未通过(1)。
