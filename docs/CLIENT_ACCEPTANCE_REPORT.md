...
```powershell
.\scripts\build-client-release.ps1 -Version 3.0.81
```
...
V3.0.80 是当前已部署的生产基线；V3.0.81 是待发布候选版本。候选版本尚未打包或部署。部署前验证：

- 演示启动脚本通过。
- 演示 smoke 自检通过。
- 演示 smoke 已覆盖登录、角色跳转、固定导航选中态、审阅身份口径、同步停用说明、补图本地图片上传、补图页面无 URL 输入和生产环境禁用 demo 账号。
- 发布校验测试与文档卫生检查的结果记录在 `ops/releases/V3.0.81.md` 和本轮 SDD 报告中；不沿用历史测试计数。
- 页面脚本语法检查通过。
- 本地 `/health` 正常。
