...
```powershell
.\scripts\build-client-release.ps1 -Version 3.1.0
```
...
V3.1.0 已完成正式交付、资料核验与后台归档候选改造，本地验证、独立复审、发布包和生产验收证据统一记录在 `ops/releases/V3.1.0.md`：

- 演示启动脚本通过。
- 演示 smoke 自检通过。
- 演示 smoke 已覆盖登录、角色跳转、固定导航选中态、审阅身份口径、同步停用说明、补图本地图片上传、补图页面无 URL 输入和生产环境禁用 demo 账号。
- 本地发布校验测试按实际完成情况记录；正式发布包哈希和线上健康检查待 Task 8 执行后写入 `ops/releases/V3.1.0.md`。
- 页面脚本语法检查通过。
- 本地 `/health` 正常。
...
V3.1.0 的正式交付、资料核验和自动归档已进入生产发布流程；生产备份、正式包哈希、健康检查和线上验收均待 Task 8 完成，最终结论以 `ops/releases/V3.1.0.md` 的实际记录为准。
