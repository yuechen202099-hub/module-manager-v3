...
```powershell
.\scripts\build-client-release.ps1 -Version 3.2.2 -PerformanceReport .\build\release-evidence\v3.2.2-task-review.json
```
...
V3.2.0 将审阅统一收口到数据中台，并新增驾驶舱下钻和统一导出中心。完整发布证据统一记录在 `ops/releases/V3.2.0.md`：

- V3.2.0 角色、数据中台、驾驶舱下钻、导出中心、单一导出入口和发布门禁通过。
- SOP 测试、前端类型检查和正式 Vue 构建通过。
- 完整回归为 `1782 passed, 13 skipped`，独立复审结论可发布。
- 正式发布包 SHA256 为 `9448EDDCA27A36F2DF606EC1BC04A3BED05930B3D4D718E2D10381EE7FAEE6DF`。
- 生产备份、线上健康检查、权限与分页验证、服务和队列状态均已复核。
...
V3.2.0 已完成完整回归、独立复审、打包、备份、生产迁移和线上验收，当前公网生产基线为 V3.2.0。
