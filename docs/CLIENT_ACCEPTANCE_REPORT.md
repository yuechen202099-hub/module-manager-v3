...
```powershell
.\scripts\build-client-release.ps1 -Version 3.1.1
```
...
V3.1.1 修复后台条码核验和自动归档的资料组身份边界，本地验证、独立复审、发布包和后续生产验收证据统一记录在 `ops/releases/V3.1.1.md`：

- 演示启动脚本通过。
- 演示 smoke 自检通过。
- 演示 smoke 已覆盖登录、角色跳转、固定导航选中态、审阅身份口径、同步停用说明、补图本地图片上传、补图页面无 URL 输入和生产环境禁用 demo 账号。
- 本地发布校验测试按实际完成情况记录；正式发布包哈希和线上健康检查将在本轮发布后写入 `ops/releases/V3.1.1.md`。
- 页面脚本语法检查通过。
- 本地 `/health` 正常。
...
V3.1.0 的正式交付和后台缓存已在生产运行；V3.1.1 仍是补丁候选，完成备份、打包、部署和队列恢复后再更新 `ops/releases/V3.1.1.md`。
