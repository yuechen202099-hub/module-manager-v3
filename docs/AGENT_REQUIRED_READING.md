...
- 当前本地生产 worktree：`C:\Users\Administrator\.config\superpowers\worktrees\module-manager-v3\review-rephoto-workbench`
...
- 每次 BUG 修复必须将语义化 patch 版本推进 `+0.0.1`；小功能使用相同 patch 增量，大流程或主业务规则变更先请用户确认更高版本策略。
...
- 版本号由生产维护线统一分配：每次 BUG 修复严格 `+0.0.1`，小更新默认使用相同 patch 增量，大流程变更经用户确认后决定更高版本。外部团队不得自行占用正式生产版本号。
