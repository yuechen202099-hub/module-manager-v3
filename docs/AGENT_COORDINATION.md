...
| V2.5.7 | Project engineer thread | Add terminal/address search to claim task page | Ready for deployment | Local implementation and gates complete. No Alembic migration required. Browser QA verified the search input and terminal filtering; address filtering requires the running backend process to be restarted or patch-deployed so `/local-test/tasks` includes the new `address_search_text` payload. |
