...
| V2.5.3 | Project engineer thread | Project board installer KPI daily exception drilldown | Ready for review | Daily exception counts open abnormal group details for the selected installer/date. Local gates passed; production patch not yet deployed. |
| V2.5.4 | BUG fix thread | Construction upload installer should use user display name | Fixed locally | Construction upload keeps actor for permission checks and writes resolved user display name to photo creator; pending project-engineer production publish. |
