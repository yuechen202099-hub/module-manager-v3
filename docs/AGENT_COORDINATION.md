...
| V2.6.4 | Project engineer thread | Remove permanent exception/unmatched task cards from review and construction pages | Ready for deployment | Local implementation and gates complete. Scope: hide/remove the always-visible exception/unmatched task entry cards from `/task-hall` and `/construction`; keep underlying APIs/data and terminal workflows unchanged. No database migration planned. |
