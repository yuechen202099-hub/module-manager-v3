...
| V2.6.0 | Project engineer thread | Installer KPI 2-hour efficiency model with completion trend and address-weight drilldown | Ready for deployment | Code, docs, Vue build, strict-native gate, and backend API tests complete. No database migration. Scope: two-hour KPI segments, completion line, per-effective-hour output, clickable address list, address clustering/difficulty weights, project-board chart UI, version metadata, tests, and docs. |
