...
| V2.6.3 | BUG fix thread + project engineer deploy thread | Installer KPI same-building address clustering rule | Ready for deployment | Backend rule and regression test complete. Groups same `号` / same `弄+号` addresses together for KPI address weight, including public equipment in the same building. No database migration. |
