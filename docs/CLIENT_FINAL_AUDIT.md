...
| 发布包 | `build/server-release/module-manager-v2-server-3.0.82.zip` | 待发布 |
...
结果：部署前按 V3.0.82 机器版本源执行；最终结果写入 `ops/releases/V3.0.82.md`。该记录在生产执行完成前保持待发布，不填写虚构的上线证据。

```powershell
.\.venv\Scripts\python.exe -m pytest -q
```

结果：不沿用历史测试计数；本轮发布校验测试结果写入 `ops/releases/V3.0.82.md`。
