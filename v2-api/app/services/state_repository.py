...
def _only_missing_collector_photo_exception_clause():
    ...
    ignored_reasons = cast(
        literal(json.dumps(sorted(data_center_service.DASHBOARD_IGNORED_EXCEPTION_REASONS), ensure_ascii=False)),
        JSONB,
    )
    return and_(
        func.jsonb_typeof(reasons) == "array",
        func.jsonb_array_length(reasons) > 0,
