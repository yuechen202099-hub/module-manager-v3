...
assert.match(releaseNotes, /APP_VERSION\s*=\s*'3\.0\.25'/, 'APP_VERSION must be 3.0.25')
...
assert.match(releaseNotes, /管理员顶部新增|更新内容/, 'release notes must be written in Chinese')
...
assert.match(webIndex, /Module Manager V3\.0\.25/, 'web HTML title must be 3.0.25')
...
assert.match(legacyLayout, /V3\.0\.25/, 'legacy layout must show the new version')

const packageJson = JSON.parse(read('v2-web/package.json'))
assert.equal(packageJson.version, '3.0.25', 'web package version must be 3.0.25')

const pyproject = read('v2-api/pyproject.toml')
assert.match(pyproject, /version = "3\.0\.25"/, 'API package version must be 3.0.25')
assert.match(pyproject, /V3\.0\.25/, 'API description must mention V3.0.25')

const mainPy = read('v2-api/app/main.py')
assert.match(mainPy, /version="3\.0\.25"/, 'FastAPI version must be 3.0.25')
