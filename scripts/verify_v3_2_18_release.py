...
VERSION = "3.2.18"
DISPLAY_VERSION = "V3.2.18"
DEPLOYED_BASELINE = "V3.2.17"
MAINTENANCE_BRANCH = "production/V3/3.2.18"
MIGRATION_REVISION = "20260824_0016"
RELEASE_PATH = "ops/releases/V3.2.18.md"
BASELINE_RELEASE_PATH = "ops/releases/V3.2.17.md"
ARCHIVE_PATH = "build/server-release/module-manager-v2-server-3.2.18.zip"
PRODUCTION_RECORD_SHA256 = "eb3007e5ec2b63d4a56ee80b76864d451faed55c511638be86007ca61bd6807d"
OPERATOR_PRODUCTION_MARKERS = (
    "当前生产 release：`/opt/module-manager-v2/releases/v3.2.17-20260829T095416Z`",
    "当前回滚 release：`/opt/module-manager-v2/releases/v3.2.16-20260829T020456Z`",
    "当前生产提交：`16855540ba4679c43638f5c86f37370f8be2344c`",
)
...
VERSION_SURFACES = {
    "v2-api/app/main.py": 'version="3.2.18"',
    "v2-api/app/services/ops_status.py": 'return "3.2.18"',
    "v2-api/pyproject.toml": 'version = "3.2.18"',
    "v2-api/scripts/verify_v3_1_release.py": 'EXPECTED_VERSION = "3.2.18"',
    "v2-api/tests/test_v3_1_release.py": 'EXPECTED_VERSION = "3.2.18"',
    "v2-web/index.html": "Module Manager V3.2.18",
    "v2-web/package.json": '"version": "3.2.18"',
    "v2-web/src/components/AppLayout.vue": "V3.2.18",
    "v2-web/src/constants/releaseNotes.ts": "version: 'V3.2.18'",
    "v2-web/src/version.json": '"version":"3.2.18"',
    "RELEASE_MANIFEST.md": "- Version: 3.2.18",
}
    ...
    *,
    ...
    for relative_path in REQUIRED_FILES:
        if not (root / relative_path).is_file():
            failures.append(f"{relative_path}: required V3.2.18 file is missing")
    ...
    for path, markers in FEATURE_MARKERS.items():
        label = "V3.2.18 rephoto and preview contract"
    ...
    for path, markers in TASK_FEATURE_MARKERS.items():
        _require_markers(root, path, markers, "V3.2.18 Task 1-4 feature contract", failures)
    ...
    for path, markers in V3216_FEATURE_MARKERS.items():
        _require_markers(root, path, markers, "V3.2.18 feature contract", failures)
    ...
    for path, markers in V3217_FEATURE_MARKERS.items():
        _require_markers(root, path, markers, "V3.2.18 anomaly/export feature contract", failures)
    ...
    for path, markers in V3218_FEATURE_MARKERS.items():
        _require_markers(root, path, markers, "V3.2.18 approved-exception closure", failures)
    for path, marker, label in V3218_TEST_MARKERS:
        _require_markers(root, path, (marker,), label, failures)

    release_tool_markers = {
        "scripts/build-client-release.ps1": (
            '[string]$Version = "3.2.18"',
            MAINTENANCE_BRANCH,
            "scripts\\verify_v3_2_16_release.py",
            ...
            "verify_v3_2_18_release.py --phase package --package $zipPath --expected-source-commit $sourceCommit",
        ),
        "scripts/verify-client-release.py": (
            '"scripts/verify_v3_2_16_release.py"',
            '"scripts/test_verify_v3_2_16_release.py"',
        ...
        "scripts/verify_release_sop.py": (
            'with_name("verify_v3_2_18_release.py")',
            'candidate == "V3.2.18"',
            '"scripts/verify_v3_2_18_release.py"',
            '"scripts/verify_v3_2_17_release.py"',
            '"scripts/verify_v3_2_16_release.py"',
    ...
    for path, markers in release_tool_markers.items():
        _require_markers(root, path, markers, "V3.2.18 release gate", failures)
...
def _check_package(
    ...
    if expected_source_commit is None:
        failures.append("package phase requires --expected-source-commit for V3.2.18")
        return
    ...
    if re.fullmatch(r"[0-9a-f]{40}", normalized_expected_commit) is None:
        failures.append("V3.2.18 expected source commit must be one lowercase 40-character Git commit")
        return
    if package_path is None:
        failures.append("package phase requires --package for V3.2.18")
        return
    package = Path(package_path)
    if not package.is_file() or package.stat().st_size <= 0:
        failures.append(f"V3.2.18 package is missing or empty: {package}")
        return
    try:
        with ZipFile(package) as archive:
            ...
            if "SOURCE_COMMIT" not in name_set:
                failures.append("V3.2.18 package requires SOURCE_COMMIT")
            else:
                source_commit = archive.read("SOURCE_COMMIT").decode("ascii", errors="replace").strip()
                if re.fullmatch(r"[0-9a-f]{40}", source_commit) is None:
                    failures.append("V3.2.18 SOURCE_COMMIT must be one lowercase 40-character Git commit")
            if "RELEASE_MANIFEST.md" not in name_set:
                failures.append("V3.2.18 package requires RELEASE_MANIFEST.md")
            else:
                manifest_bytes = archive.read("RELEASE_MANIFEST.md")
                if b"\r" in manifest_bytes:
                    failures.append("V3.2.18 RELEASE_MANIFEST.md must use LF line endings")
                manifest = manifest_bytes.decode("utf-8", errors="replace")
                versions = re.findall(r"(?m)^- Version:\s*(\S+)\s*$", manifest)
                if versions != [VERSION]:
                    failures.append(f"V3.2.18 package manifest must contain exactly Version {VERSION}")
    except (BadZipFile, OSError) as exc:
        failures.append(f"V3.2.18 package cannot be opened: {exc}")
        return
    ...
    try:
        ...
        failures.append(f"V3.2.18 package verification failed: {exc}")
...
def _check_attestation(
    ...
    path_patterns = {
        "Backup directory": r"/opt/module-manager-v2/backups/v3\.2\.18-\d{8}T\d{6}Z",
        "Release directory": r"/opt/module-manager-v2/releases/v3\.2\.18-\d{8}T\d{6}Z",
        "Rollback directory": r"/opt/module-manager-v2/releases/v3\.2\.17-\d{8}T\d{6}Z",
    }
    ...
    for field in ("Local health", "Public health"):
        ...
        if (
            len(values) != 1
            or re.fullmatch(r"HTTP\s+200\s+version\s+3\.2\.18", values[0], re.IGNORECASE) is None
        ):
            failures.append(f"{RELEASE_PATH}: {field} must prove HTTP 200 for version 3.2.18")
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Verify the V3.2.18 source, package, or attestation contract.")
    ...
    if failures:
        ...
        return 1
    print(f"[OK] V3.2.18 {args.phase} release contract")
    return 0
