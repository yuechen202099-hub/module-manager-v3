...
def test_cli_parses_version_and_rejects_unknown_arguments() -> None:
    ...
    assert verifier.parse_args(["--version", "V3.2.1"]).version == "V3.2.1"
    with pytest.raises(SystemExit):
        verifier.parse_args(["--version", "V3.2.1", "--unknown"])
...
def test_cli_version_must_match_the_release_candidate() -> None:
    ...
    assert verifier.validate_requested_candidate_version("V3.2.1", agents) == "V3.2.1"
...
def test_v321_manifest_records_the_release_candidate_package() -> None:
    manifest = (ROOT / "RELEASE_MANIFEST.md").read_text(encoding="utf-8")

    assert "- Package: `build/server-release/module-manager-v2-server-3.2.1.zip`" in manifest
    assert "- Name: `module-manager-v2-server-3.2.1.zip`" in manifest
...
def test_round5_vue_app_version_uses_one_machine_source_and_entry_marker() -> None:
    ...
    assert json.loads(source_path.read_text(encoding="utf-8")) == {"version": "3.2.1"}
