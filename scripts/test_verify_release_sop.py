...
def test_v3217_sop_inputs_bind_current_and_historical_release_gates() -> None:
...
def test_v3217_release_contract_is_registered_for_source_and_attestation() -> None:
    verifier = load_verifier()

    assert "scripts/verify_v3_2_17_release.py" in verifier.RELEASE_INPUTS
    assert "scripts/test_verify_v3_2_17_release.py" in verifier.RELEASE_INPUTS
    assert "ops/releases/V3.2.17.md" in verifier.RELEASE_INPUTS

    verifier.verify_current_release_phase("source", "V3.2.17")


def test_v3217_sop_attestation_forwards_verified_package_and_source_commit(monkeypatch) -> None:
    verifier = load_verifier()
    ...
    verifier.verify_current_release_phase(
        "attestation",
        "V3.2.17",
        package_path=ROOT / "build/server-release/module-manager-v2-server-3.2.17.zip",
    ...
    assert captured == [[
        ...
        "attestation",
        "--package",
        str(ROOT / "build/server-release/module-manager-v2-server-3.2.17.zip"),
...
def test_parses_current_deployed_baseline_and_release_candidate_markers() -> None:
    ...
    assert verifier.deployed_production_baseline(agents) == "V3.2.16"
    assert verifier.release_candidate(agents) == "V3.2.17"
...
def test_cli_parses_version_and_rejects_unknown_arguments() -> None:
    ...
    args = verifier.parse_args(
        [
            ...
            "attestation",
            "--package",
            "build/server-release/module-manager-v2-server-3.2.17.zip",
            "--expected-source-commit",
            "1" * 40,
        ]
    )
    assert args.version == "V3.2.17"
    assert args.phase == "attestation"
    assert args.package == Path("build/server-release/module-manager-v2-server-3.2.17.zip")
    assert args.expected_source_commit == "1" * 40
    with pytest.raises(SystemExit):
        verifier.parse_args(["--version", "V3.2.17", "--phase", "attestation", "--unknown"])
    with pytest.raises(SystemExit):
        verifier.parse_args(["--version", "V3.2.17"])
...
def test_cli_version_must_match_the_release_candidate() -> None:
    ...
    assert verifier.validate_requested_candidate_version("V3.2.17", agents) == "V3.2.17"
...
def test_cli_verifies_the_current_source_contract() -> None:
    ...
    assert verifier.main(["--version", "V3.2.17", "--phase", "source"]) == 0
...
def test_cli_rejects_current_pending_candidate_as_attestation() -> None:
    ...
    with pytest.raises(AssertionError, match="attestation"):
        verifier.main(["--version", "V3.2.17", "--phase", "attestation"])
...
def test_rejects_wrong_release_candidate_maintenance_branch() -> None:
    ...
    agents = (ROOT / "AGENTS.md").read_text(encoding="utf-8").replace(
        "production/V3/3.2.17", "production/V3/3.0.81"
    )
...
def test_v323_manifest_records_the_release_candidate_package() -> None:
    ...
    assert "- Package: `build/server-release/module-manager-v2-server-3.2.17.zip`" in manifest
    assert "- Name: `module-manager-v2-server-3.2.17.zip`" in manifest
    assert "- Version: 3.2.17" in manifest
...
def test_round5_vue_app_version_uses_one_machine_source_and_entry_marker() -> None:
    ...
    assert json.loads(source_path.read_text(encoding="utf-8")) == {"version": "3.2.17"}
