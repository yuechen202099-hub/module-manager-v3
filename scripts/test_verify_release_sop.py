...
def test_v3220_sop_inputs_bind_current_and_historical_release_gates() -> None:
...
def test_current_release_contract_is_registered_for_source_and_attestation() -> None:
    verifier = load_verifier()

    assert "scripts/verify_v3_2_22_release.py" in verifier.RELEASE_INPUTS
    assert "scripts/test_verify_v3_2_22_release.py" in verifier.RELEASE_INPUTS
    assert "ops/releases/V3.2.22.md" in verifier.RELEASE_INPUTS
    assert "ops/releases/V3.2.23.md" in verifier.RELEASE_INPUTS
    assert "scripts/verify_v3_2_24_release.py" in verifier.RELEASE_INPUTS
    assert "scripts/test_verify_v3_2_24_release.py" in verifier.RELEASE_INPUTS
    assert "ops/releases/V3.2.24.md" in verifier.RELEASE_INPUTS
    assert "scripts/verify_v3_2_25_release.py" in verifier.RELEASE_INPUTS
    assert "scripts/test_verify_v3_2_25_release.py" in verifier.RELEASE_INPUTS
    assert "ops/releases/V3.2.25.md" in verifier.RELEASE_INPUTS
    assert "scripts/verify_v3_2_26_release.py" in verifier.RELEASE_INPUTS
    assert "scripts/test_verify_v3_2_26_release.py" in verifier.RELEASE_INPUTS
    assert "ops/releases/V3.2.26.md" in verifier.RELEASE_INPUTS
    assert "scripts/verify_v3_2_27_release.py" in verifier.RELEASE_INPUTS
    assert "scripts/test_verify_v3_2_27_release.py" in verifier.RELEASE_INPUTS
    assert "ops/releases/V3.2.27.md" in verifier.RELEASE_INPUTS
    assert "scripts/verify_v3_2_28_release.py" in verifier.RELEASE_INPUTS
    assert "scripts/test_verify_v3_2_28_release.py" in verifier.RELEASE_INPUTS
    assert "ops/releases/V3.2.28.md" in verifier.RELEASE_INPUTS
    assert "scripts/verify_v3_2_29_release.py" in verifier.RELEASE_INPUTS
    assert "scripts/test_verify_v3_2_29_release.py" in verifier.RELEASE_INPUTS
    assert "ops/releases/V3.2.29.md" in verifier.RELEASE_INPUTS

    verifier.verify_current_release_phase("source", "V3.2.29")


def test_v3221_sop_attestation_forwards_verified_package_and_source_commit(monkeypatch) -> None:
    verifier = load_verifier()
    ...
    monkeypatch.setattr(verifier.importlib.util, "spec_from_file_location", lambda *_: Spec())
    monkeypatch.setattr(verifier.importlib.util, "module_from_spec", lambda _: Module())

    verifier.verify_current_release_phase(
        "attestation",
        "V3.2.21",
        package_path=ROOT / "build/server-release/module-manager-v2-server-3.2.21.zip",
        expected_source_commit="02fbe4b5d55b68708683303361389234be3807af",
    )
    ...
    assert captured == [[
        ...
        "attestation",
        "--package",
        str(ROOT / "build/server-release/module-manager-v2-server-3.2.21.zip"),
        "--expected-source-commit",
        "02fbe4b5d55b68708683303361389234be3807af",
    ]]
...
def test_v3217_attested_record_is_accepted_as_v3218_baseline_and_is_byte_locked() -> None:
    verifier = load_verifier()
    record = (ROOT / "ops/releases/V3.2.17.md").read_text(encoding="utf-8")

    assert verifier.verified_v3217_attested_baseline_is_documented(record, "V3.2.17")
    verifier.release_record_matches_lifecycle_state(
        record,
        "V3.2.17",
        "V3.2.17",
    ...
    with pytest.raises(AssertionError, match="byte-identical"):
        verifier.verified_v3217_attested_baseline_is_documented(
            record + "\nmutated\n",
            "V3.2.17",
        )


def test_v3218_attested_record_is_accepted_as_v3219_baseline_and_is_byte_locked() -> None:
    verifier = load_verifier()
    record = (ROOT / "ops/releases/V3.2.18.md").read_text(encoding="utf-8")

    assert verifier.verified_v3218_attested_baseline_is_documented(record, "V3.2.18")
    verifier.release_record_matches_lifecycle_state(
        record,
        "V3.2.18",
        "V3.2.18",
    ...
    with pytest.raises(AssertionError, match="byte-identical"):
        verifier.verified_v3218_attested_baseline_is_documented(
            record + "\nmutated\n",
            "V3.2.18",
        )


def test_v3219_attested_record_is_accepted_as_v3220_baseline_and_is_byte_locked() -> None:
    verifier = load_verifier()
    record = (ROOT / "ops/releases/V3.2.19.md").read_text(encoding="utf-8")

    assert verifier.verified_v3219_attested_baseline_is_documented(record, "V3.2.19")
    verifier.release_record_matches_lifecycle_state(
        record,
        "V3.2.19",
        "V3.2.19",
    ...
    with pytest.raises(AssertionError, match="byte-identical"):
        verifier.verified_v3219_attested_baseline_is_documented(
            record + "\nmutated\n",
            "V3.2.19",
        )


def test_v3220_attested_record_is_accepted_as_v3221_baseline_and_is_byte_locked() -> None:
    verifier = load_verifier()
    record = (ROOT / "ops/releases/V3.2.20.md").read_text(encoding="utf-8")

    assert verifier.verified_v3220_attested_baseline_is_documented(record, "V3.2.20")
    verifier.release_record_matches_lifecycle_state(
        record,
        "V3.2.20",
        "V3.2.20",
    ...
    with pytest.raises(AssertionError, match="byte-identical"):
        verifier.verified_v3220_attested_baseline_is_documented(
            record + "\nmutated\n",
            "V3.2.20",
        )


def test_v3221_attested_record_is_accepted_as_v3222_baseline_and_is_byte_locked() -> None:
    verifier = load_verifier()
    record = (ROOT / "ops/releases/V3.2.21.md").read_text(encoding="utf-8")

    assert verifier.verified_v3221_attested_baseline_is_documented(record, "V3.2.21")
    verifier.release_record_matches_lifecycle_state(
        record,
        "V3.2.21",
        "V3.2.21",
    ...
    with pytest.raises(AssertionError, match="byte-identical"):
        verifier.verified_v3221_attested_baseline_is_documented(
            record + "\nmutated\n",
            "V3.2.21",
        )
...
def test_parses_current_deployed_baseline_and_release_candidate_markers() -> None:
    ...
    assert verifier.deployed_production_baseline(agents) == "V3.2.28"
    assert verifier.release_candidate(agents) == "V3.2.29"
...
def test_v3213_attested_baseline_is_byte_locked() -> None:
    verifier = load_verifier()
    record = (ROOT / "ops/releases/V3.2.13.md").read_text(encoding="utf-8")

    assert verifier.verified_v3213_attested_baseline_is_documented(record, "V3.2.13")
    with pytest.raises(AssertionError, match="byte-identical"):
        verifier.verified_v3213_attested_baseline_is_documented(
            record + "\nmutated\n",
            "V3.2.13",
        )
...
def test_cli_parses_version_and_rejects_unknown_arguments() -> None:
    ...
    args = verifier.parse_args(
        [
            ...
            "attestation",
            "--package",
            "build/server-release/module-manager-v2-server-3.2.21.zip",
            "--expected-source-commit",
            "1" * 40,
        ]
    )
    assert args.version == "V3.2.21"
    assert args.phase == "attestation"
    assert args.package == Path("build/server-release/module-manager-v2-server-3.2.21.zip")
    assert args.expected_source_commit == "1" * 40
    with pytest.raises(SystemExit):
        verifier.parse_args(["--version", "V3.2.21", "--phase", "attestation", "--unknown"])
    with pytest.raises(SystemExit):
        verifier.parse_args(["--version", "V3.2.21"])
...
def test_cli_version_must_match_the_release_candidate() -> None:
    ...
    assert verifier.validate_requested_candidate_version("V3.2.29", agents) == "V3.2.29"
...
def test_cli_verifies_the_current_source_contract() -> None:
    ...
    assert verifier.main(["--version", "V3.2.29", "--phase", "source"]) == 0
...
def test_cli_rejects_current_pending_candidate_as_attestation() -> None:
    ...
    with pytest.raises(AssertionError, match="attestation"):
        verifier.main(["--version", "V3.2.29", "--phase", "attestation"])
...
def test_rejects_wrong_release_candidate_maintenance_branch() -> None:
    ...
    agents = (ROOT / "AGENTS.md").read_text(encoding="utf-8").replace(
        "production/V3/3.2.29", "production/V3/3.0.81"
    )
...
def test_current_v3211_source_phase_is_checked() -> None:
    verifier = load_verifier()

    verifier.verify_current_release_phase("source")


def test_v3222_attested_production_baseline_is_accepted_without_rewriting_history() -> None:
    verifier = load_verifier()
    record = (ROOT / "ops" / "releases" / "V3.2.22.md").read_text(encoding="utf-8")

    verifier.deployed_release_record_is_verified(record, "V3.2.22")


def test_v3225_attested_baseline_is_byte_locked_for_v3226() -> None:
    verifier = load_verifier()
    record = (ROOT / "ops" / "releases" / "V3.2.25.md").read_text(encoding="utf-8")

    assert verifier.verified_v3225_attested_baseline_is_documented(record, "V3.2.25")
    with pytest.raises(AssertionError, match="byte-identical"):
        verifier.verified_v3225_attested_baseline_is_documented(
            record + "\nmutated\n",
            "V3.2.25",
        )
...
def test_manifest_records_the_current_release_candidate_package() -> None:
    manifest = (ROOT / "RELEASE_MANIFEST.md").read_text(encoding="utf-8")

    assert "- Package: `build/server-release/module-manager-v2-server-3.2.29.zip`" in manifest
    assert "- Name: `module-manager-v2-server-3.2.29.zip`" in manifest
    assert "- Version: 3.2.29" in manifest
...
def test_release_record_status_ignores_prose_starting_with_release() -> None:
    verifier = load_verifier()
    record = valid_deployed_record() + (
        "\nRelease retention kept five releases: V3.2.25 through V3.2.21.\n"
    )

    assert verifier.release_record_status(record) == "deployed"
...
def test_round5_vue_app_version_uses_one_machine_source_and_entry_marker() -> None:
    ...
    assert json.loads(source_path.read_text(encoding="utf-8")) == {"version": "3.2.29"}
