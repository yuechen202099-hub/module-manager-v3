...
def test_parses_current_deployed_baseline_and_release_candidate_markers() -> None:
    ...
    assert verifier.deployed_production_baseline(agents) == "V3.2.5"
    assert verifier.release_candidate(agents) == "V3.2.6"


def test_cli_parses_version_and_rejects_unknown_arguments() -> None:
    verifier = load_verifier()

    args = verifier.parse_args(["--version", "V3.2.6", "--phase", "attestation"])
    assert args.version == "V3.2.6"
    assert args.phase == "attestation"
    with pytest.raises(SystemExit):
        verifier.parse_args(["--version", "V3.2.6", "--phase", "attestation", "--unknown"])
    with pytest.raises(SystemExit):
        verifier.parse_args(["--version", "V3.2.6"])
...
def test_cli_version_must_match_the_release_candidate() -> None:
    ...
    assert verifier.validate_requested_candidate_version("V3.2.6", agents) == "V3.2.6"
...
def test_cli_verifies_the_current_release_contract() -> None:
    ...
    assert verifier.main(["--version", "V3.2.6", "--phase", "source"]) == 0
...
def test_rejects_wrong_release_candidate_maintenance_branch() -> None:
    ...
    agents = (ROOT / "AGENTS.md").read_text(encoding="utf-8").replace(
        "production/V3/3.2.6", "production/V3/3.0.81"
    )
...
def test_current_v326_source_phase_is_checked() -> None:
    verifier = load_verifier()
...
@pytest.mark.parametrize(
    ...
    (
        "- Deployed production baseline: `V3.2.5`; V3.2.6 was deployed today.",
        "- 当前已部署生产版本：`V3.2.5`；V3.2.6 已部署到生产环境。",
    ),
)
...
def test_v323_manifest_records_the_release_candidate_package() -> None:
    ...
    assert "- Package: `build/server-release/module-manager-v2-server-3.2.6.zip`" in manifest
    assert "- Name: `module-manager-v2-server-3.2.6.zip`" in manifest
    assert "- Version: 3.2.6" in manifest
...
def test_round5_vue_app_version_uses_one_machine_source_and_entry_marker() -> None:
    ...
    assert json.loads(source_path.read_text(encoding="utf-8")) == {"version": "3.2.6"}
