...
def test_parses_current_deployed_baseline_and_release_candidate_markers() -> None:
    ...
    assert verifier.deployed_production_baseline(agents) == "V3.2.1"
    assert verifier.release_candidate(agents) == "V3.2.2"


def test_cli_parses_version_and_rejects_unknown_arguments() -> None:
    verifier = load_verifier()

    assert verifier.parse_args(["--version", "V3.2.2"]).version == "V3.2.2"
    with pytest.raises(SystemExit):
        verifier.parse_args(["--version", "V3.2.2", "--unknown"])
...
def test_cli_version_must_match_the_release_candidate() -> None:
    ...
    assert verifier.validate_requested_candidate_version("V3.2.2", agents) == "V3.2.2"
...
def test_v322_manifest_records_the_release_candidate_package() -> None:
    manifest = (ROOT / "RELEASE_MANIFEST.md").read_text(encoding="utf-8")

    assert "- Package: `build/server-release/module-manager-v2-server-3.2.2.zip`" in manifest
    assert "- Name: `module-manager-v2-server-3.2.2.zip`" in manifest
    assert "- Version: 3.2.2" in manifest
...
def test_round5_vue_app_version_uses_one_machine_source_and_entry_marker() -> None:
    ...
    assert json.loads(source_path.read_text(encoding="utf-8")) == {"version": "3.2.2"}
