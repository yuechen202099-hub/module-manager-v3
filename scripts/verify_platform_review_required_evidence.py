...
def create_project(client: TestClient) -> str:
    response = client.post(
        "/projects",
        json={
            "name": "review required evidence guard",
        ...
        try:
            ...
            cached_response = client.post(
                ...
                json={
                    ...
                    "covered_photo_slots": [],
                    "client_batch_id": "review-incomplete",
                },
            )
            require(cached_response.status_code == 200, "cached incomplete draft should be allowed")

            approve_incomplete_response = client.post(
                f"/projects/{project_id}/review/work-orders/{work_order_id}/actions",
                json={"actor": "reviewer", "action": "approved", "note": ""},
            )
            require(approve_incomplete_response.status_code == 400, "incomplete evidence must not be approved")
            detail = str(approve_incomplete_response.json().get("detail") or "")
            require("新终端号" in detail, "missing required field label not reported")
            ...
            complete_response = client.post(
                ...
                json={
                    ...
                    "client_batch_id": "review-complete",
                },
            )
            require(complete_response.status_code == 200, "complete collection should submit")

            approve_complete_response = client.post(
                f"/projects/{project_id}/review/work-orders/{work_order_id}/actions",
                json={"actor": "reviewer", "action": "approved", "note": "ok"},
            )
            require(approve_complete_response.status_code == 200, "complete evidence should be approved")
            require(approve_complete_response.json()["data"]["review_status"] == "approved", "review status not approved")
        finally:
            reset_project_drafts(remove_store=True)
            reset_platform_runtime_files()
            configure_project_draft_store_path(None)

    print("[OK] platform review required evidence is enforced")
