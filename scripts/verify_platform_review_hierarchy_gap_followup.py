...
def create_terminal_project(client: TestClient) -> str:
    response = client.post(
        "/projects",
        json={
            "name": "review hierarchy gap followup",
...
def create_external_completed_batch(client: TestClient, project_id: str) -> str:
    rows = [
    ...
    response = client.post(
        ...
        files={
            "file": (
                "external-completed-review-gap.xlsx",
                workbook_bytes(rows),
                "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            )
        },
    )
    require(response.status_code == 200, f"external completed import batch failed: {response.text}")
    return response.json()["data"]["job_id"]


def create_and_execute_work_order_task(client: TestClient, project_id: str, batch_id: str) -> None:
    ...
    task_id = task_response.json()["data"]["job_id"]
...
def main() -> None:
    with TemporaryDirectory() as temp_dir:
        ...
        try:
            ...
            batch_id = create_external_completed_batch(client, project_id)
            create_and_execute_work_order_task(client, project_id, batch_id)
            ...
            review_work_order = items[0]
            gap_items = review_work_order.get("review_hierarchy_gap_items")
            require(isinstance(gap_items, list), "review payload must expose review_hierarchy_gap_items")
            require(len(gap_items) == 3, "review hierarchy gap items must include all triggered child evidence")
            labels = {str(item.get("field_label") or "") for item in gap_items}
            require("旧通讯模块号" in labels, "old accessory device hierarchy gap missing from review")
            require("新通讯模块号" in labels, "new accessory device hierarchy gap missing from review")
            require("新旧模块照片" in labels, "photo hierarchy gap missing from review")
        finally:
            reset_project_drafts(remove_store=True)
            reset_platform_runtime_files()
            configure_project_draft_store_path(None)

    print("[OK] review hierarchy gap follow-up is preserved")
