...
- Release candidate: `V3.2.14`.
- Release-candidate maintenance branch: `production/V3/3.2.14`.
- 当前已部署生产版本：`V3.2.13`。
- 当前发布候选版本：`V3.2.14`。
- 当前候选维护分支：`production/V3/3.2.14`。
"""
PENDING_RELEASE_RECORD = """# V3.2.14 Production Release Record
...
- safe constructed meter rephoto survives unrelated incomplete rows
- manual collector demand is current-project scoped and atomic
- completed meter and collector rows hide by default
- present collectors may reuse one source-group collector photo
...
def test_v3214_package_contract_inherits_v3213_and_registers_current_gates() -> None:
    ...
    assert verifier.V3213_CONTRACT_INPUTS < verifier.V3214_CONTRACT_INPUTS
    assert expected <= verifier.V3214_CONTRACT_INPUTS
...
def write_release_archive(
    ...
    archive_path: Path,
    *,
    manifest_version: str | None = "3.2.14",
    manifest_versions: list[str] | None = None,
    static_version: str = "3.2.14",
    ...
    supported_version = manifest_version in {"3.2.8", "3.2.10", "3.2.11", "3.2.12", "3.2.13", "3.2.14"}
    ...
    contents = {
        ...
        V3214_RELEASE_RECORD: release_record,
...
def test_release_builder_default_version_is_candidate_semantic_version() -> None:
    ...
    assert '[string]$Version = "3.2.14"' in build_script
...
def test_release_builder_rejects_non_candidate_version_before_output_mutation(
    ...
    assert "Expected exactly 3.2.14" in output
...
def test_release_builder_guards_candidate_version_before_output_mutation() -> None:
    ...
    candidate_guard = build_script.index("Expected exactly 3.2.14")
...
def test_v3214_build_sequence_executes_current_and_compatible_release_gates_only() -> None:
    build_script = (ROOT / "scripts" / "build-client-release.ps1").read_text(encoding="utf-8")
...
def test_v3214_builder_executes_contract_tests_and_stops_on_failure(tmp_path: Path) -> None:
    build_script = (ROOT / "scripts" / "build-client-release.ps1").read_text(encoding="utf-8")
    gate_start_marker = 'Write-Host "Running V3.2.14 contract tests..."'
    next_gate_marker = (
    ...
    harness = tmp_path / "run-v3214-contract-gate.ps1"
    ...
    assert "V3.2.14 contract tests failed" in output
    assert not continued.exists()
    invocation = invocation_log.read_text(encoding="utf-8")
    assert "-m pytest" in invocation
    assert ".\\scripts\\test_verify_v3_2_14_release.py -q" in invocation


def test_v3214_archive_rejects_an_unapproved_alembic_revision(tmp_path: Path) -> None:
    verifier = load_verifier()
    archive_path = tmp_path / "v3214-unapproved-migration.zip"
    write_release_archive(
        verifier,
        archive_path,
        content_overrides={
...
def test_all_copied_operational_documents_reject_stale_candidate_markers() -> None:
    ...
    for document_path in document_paths:
        ...
        verifier.verify_release_markdown_text(document_path, content, "3.2.14")
...
def test_archive_source_and_runtime_version_artifacts_must_match(tmp_path: Path) -> None:
    ...
    with pytest.raises(AssertionError, match="V3.2.14 archive source contract|source and built runtime versions"):
        verifier.verify_package(archive_path)
...
def test_archive_rejects_stale_entry_bundle_despite_current_sidecars(tmp_path: Path) -> None:
    ...
    write_release_archive(
        verifier,
        archive_path,
        runtime_version="3.2.14",
        source_version="3.2.14",
        entry_version="3.2.4",
        unrelated_static_version="3.2.14",
        unrelated_chunk_entry_version="3.2.14",
    )
...
@pytest.mark.parametrize(
    ...
    [
        (
            "star-package",
            "Package",
            "* Package: module-manager-v2-server-3.2.14.zip",
        ),
...
def test_archive_rejects_pending_candidate_with_unversioned_claim_and_complete_evidence(
    ...
    record = (
        PENDING_RELEASE_RECORD.replace("- V3.2.14 has not been deployed to production.\n", "")
...
def test_archive_rejects_mutated_v3213_deployed_baseline_record(tmp_path: Path) -> None:
    verifier = load_verifier()
    ...
    write_release_archive(
        ...
        archive_path,
        content_overrides={
            V3213_RELEASE_RECORD: deployed_release_record("3.2.13", "Status: deployed")
        },
    )
