...
V329_RELEASE_RECORD = "ops/releases/V3.2.9.md"
V3210_RELEASE_RECORD = "ops/releases/V3.2.10.md"
V3211_RELEASE_RECORD = "ops/releases/V3.2.11.md"
V3212_RELEASE_RECORD = "ops/releases/V3.2.12.md"
...
V3217_RELEASE_RECORD = "ops/releases/V3.2.17.md"
V3218_RELEASE_RECORD = "ops/releases/V3.2.18.md"
V3219_RELEASE_RECORD = "ops/releases/V3.2.19.md"
V3220_RELEASE_RECORD = "ops/releases/V3.2.20.md"
V3221_RELEASE_RECORD = "ops/releases/V3.2.21.md"
V3222_RELEASE_RECORD = "ops/releases/V3.2.22.md"
V3223_RELEASE_RECORD = "ops/releases/V3.2.23.md"
V3224_RELEASE_RECORD = "ops/releases/V3.2.24.md"
V3225_RELEASE_RECORD = "ops/releases/V3.2.25.md"
V3226_RELEASE_RECORD = "ops/releases/V3.2.26.md"
V3227_RELEASE_RECORD = "ops/releases/V3.2.27.md"
V3228_RELEASE_RECORD = "ops/releases/V3.2.28.md"
V3229_RELEASE_RECORD = "ops/releases/V3.2.29.md"
...
V3211_AGENTS = """# Historical package fixture

- Deployed production baseline: `V3.2.10`.
- Release candidate: `V3.2.11`.
- Release-candidate maintenance branch: `production/V3/3.2.11`.
- 当前已部署生产版本：`V3.2.10`。
- 当前发布候选版本：`V3.2.11`。
- 当前候选维护分支：`production/V3/3.2.11`。
"""
V3211_PENDING_RELEASE_RECORD = """# V3.2.11 Production Release Record
...
- V3.2.11 has not been deployed to production.
- unified terminal review workbench
- unconstructed meters do not block
- constructed meters lock the whole terminal until review-ready
- project/team-scoped one-time collector allocation
- V3.2.9 Quagga-first scanner regression retained
- schema remains 20260824_0016
...
- Release candidate: `V3.2.29`.
- Release-candidate maintenance branch: `production/V3/3.2.29`.
- 当前已部署生产版本：`V3.2.28`。
- 当前发布候选版本：`V3.2.29`。
- 当前候选维护分支：`production/V3/3.2.29`。
"""
PENDING_RELEASE_RECORD = (ROOT / V3229_RELEASE_RECORD).read_text(encoding="utf-8")
...
def test_v3222_package_contract_inherits_v3221_and_registers_current_gates() -> None:
    ...
    assert verifier.V3221_CONTRACT_INPUTS < verifier.V3222_CONTRACT_INPUTS
    assert verifier.V3222_CONTRACT_INPUTS < verifier.V3223_CONTRACT_INPUTS
    assert verifier.V3223_CONTRACT_INPUTS < verifier.V3224_CONTRACT_INPUTS
    assert verifier.V3224_CONTRACT_INPUTS < verifier.V3225_CONTRACT_INPUTS
    assert expected <= verifier.V3222_CONTRACT_INPUTS
    ...
    assert verifier.required_files_for_version("3.2.17") < frozenset(verifier.REQUIRED_FILES)
    assert verifier.required_files_for_version("3.2.18") < frozenset(verifier.REQUIRED_FILES)
    assert verifier.required_files_for_version("3.2.19") < frozenset(verifier.REQUIRED_FILES)
    assert verifier.required_files_for_version("3.2.20") == frozenset(verifier.REQUIRED_FILES)
    assert verifier.required_files_for_version("3.2.21") == frozenset(
        verifier.REQUIRED_FILES | verifier.V3221_CONTRACT_INPUTS
    )
    assert verifier.required_files_for_version("3.2.22") == frozenset(
        verifier.REQUIRED_FILES | verifier.V3221_CONTRACT_INPUTS | verifier.V3222_CONTRACT_INPUTS
    )
    assert verifier.required_files_for_version("3.2.23") == frozenset(
        verifier.REQUIRED_FILES | verifier.V3221_CONTRACT_INPUTS | verifier.V3222_CONTRACT_INPUTS | verifier.V3223_CONTRACT_INPUTS
    )
    assert verifier.required_files_for_version("3.2.24") == frozenset(
        verifier.REQUIRED_FILES
        | verifier.V3221_CONTRACT_INPUTS
        | verifier.V3222_CONTRACT_INPUTS
        | verifier.V3223_CONTRACT_INPUTS
        | verifier.V3224_CONTRACT_INPUTS
    )
    assert verifier.required_files_for_version("3.2.25") == frozenset(
        verifier.REQUIRED_FILES
        | verifier.V3221_CONTRACT_INPUTS
        | verifier.V3222_CONTRACT_INPUTS
        | verifier.V3223_CONTRACT_INPUTS
        | verifier.V3224_CONTRACT_INPUTS
        | verifier.V3225_CONTRACT_INPUTS
    )
...
def test_v328_package_verifier_rejects_a_v327_candidate_archive(tmp_path: Path) -> None:
    ...
    with pytest.raises(AssertionError, match="supported archived source contract"):
        verifier.verify_package(archive_path)


@pytest.mark.parametrize(
    ("version", "required_marker", "forbidden_marker"),
    (
        (
            "3.2.8",
...
def test_missing_required_files_are_scoped_to_the_archived_version_contract(
    tmp_path: Path,
    version: str,
    required_marker: str,
    forbidden_marker: str,
) -> None:
    """A new candidate must not retroactively change an older archive's file contract."""
    verifier = load_verifier()
    archive_path = tmp_path / f"missing-version-scoped-files-{version}.zip"
    with zipfile.ZipFile(archive_path, "w") as archive:
        archive.writestr("SOURCE_COMMIT", VALID_SOURCE_COMMIT)
        archive.writestr("RELEASE_MANIFEST.md", f"- Version: {version}\n")

    with pytest.raises(AssertionError, match="Missing required release files") as error:
        verifier.verify_package(archive_path)

    message = str(error.value)
    assert required_marker in message
...
def write_release_archive(
    ...
    archive_path: Path,
    *,
    manifest_version: str | None = "3.2.29",
    manifest_versions: list[str] | None = None,
    static_version: str = "3.2.29",
    ...
    supported_version = manifest_version in {"3.2.8", "3.2.10", "3.2.11", "3.2.12", "3.2.13", "3.2.14", "3.2.15", "3.2.16", "3.2.17", "3.2.18", "3.2.19", "3.2.20", "3.2.21", "3.2.22", "3.2.23", "3.2.24", "3.2.25", "3.2.26", "3.2.27", "3.2.28", "3.2.29"}
    version_specific_required_files = (
        set(verifier.required_files_for_version(manifest_version))
        if supported_version
        else set(verifier.REQUIRED_FILES)
    )
    names = (
        version_specific_required_files
    ...
    contents = {
        ...
        V3210_RELEASE_RECORD: (ROOT / V3210_RELEASE_RECORD).read_text(encoding="utf-8"),
        V3211_RELEASE_RECORD: (ROOT / V3211_RELEASE_RECORD).read_text(encoding="utf-8"),
        ...
        V3216_RELEASE_RECORD: (ROOT / V3216_RELEASE_RECORD).read_text(encoding="utf-8"),
        V3217_RELEASE_RECORD: (ROOT / V3217_RELEASE_RECORD).read_text(encoding="utf-8"),
        V3218_RELEASE_RECORD: (ROOT / V3218_RELEASE_RECORD).read_text(encoding="utf-8"),
        V3219_RELEASE_RECORD: (ROOT / V3219_RELEASE_RECORD).read_text(encoding="utf-8"),
        V3220_RELEASE_RECORD: (ROOT / V3220_RELEASE_RECORD).read_text(encoding="utf-8"),
        V3221_RELEASE_RECORD: (ROOT / V3221_RELEASE_RECORD).read_text(encoding="utf-8"),
        V3222_RELEASE_RECORD: (ROOT / V3222_RELEASE_RECORD).read_text(encoding="utf-8"),
        V3223_RELEASE_RECORD: (ROOT / V3223_RELEASE_RECORD).read_text(encoding="utf-8"),
        V3224_RELEASE_RECORD: (ROOT / V3224_RELEASE_RECORD).read_text(encoding="utf-8"),
        V3225_RELEASE_RECORD: (ROOT / V3225_RELEASE_RECORD).read_text(encoding="utf-8"),
        V3226_RELEASE_RECORD: (ROOT / V3226_RELEASE_RECORD).read_text(encoding="utf-8"),
        V3227_RELEASE_RECORD: (ROOT / V3227_RELEASE_RECORD).read_text(encoding="utf-8"),
        V3228_RELEASE_RECORD: (ROOT / V3228_RELEASE_RECORD).read_text(encoding="utf-8"),
        V3229_RELEASE_RECORD: release_record,
    ...
    for name in names:
        if name == RUNTIME_VERSION_ARTIFACT:
            continue
        if name in contents:
            archive_contents[name] = contents[name]
        elif manifest_version in {"3.2.28", "3.2.29"} and (ROOT / name).is_file():
            archive_contents[name] = (ROOT / name).read_bytes()
        elif (
            name in KPI_SOURCE_PATHS
            or name in V328_ARCHIVE_SOURCE_FILES
            or (name.startswith("v2-api/alembic/versions/") and name.endswith(".py"))
        ):
            archive_contents[name] = (ROOT / name).read_bytes()
        else:
            archive_contents[name] = "fixture\n"
    if source_bound:
...
def test_release_builder_default_version_is_candidate_semantic_version() -> None:
    ...
    assert '[string]$Version = "3.2.29"' in build_script
...
def test_release_builder_rejects_non_candidate_version_before_output_mutation(
    ...
    assert "Expected exactly 3.2.29" in output
...
def test_release_builder_embeds_the_current_source_commit() -> None:
    ...
    assert (
        "verify_v3_2_29_release.py --phase package --package $zipPath "
...
def test_release_builder_guards_candidate_version_before_output_mutation() -> None:
    ...
    candidate_guard = build_script.index("Expected exactly 3.2.29")
...
def test_tampered_tracked_business_file_fails_source_commit_byte_binding(tmp_path: Path) -> None:
    verifier = load_verifier()
    source_commit = "2fd8c2cbe377d1316e3f97f6d7f83a53b4e08a3d"
    ...
    write_release_archive(
        verifier,
        archive_path,
        manifest_version="3.2.11",
        static_version="3.2.11",
        agents=V3211_AGENTS,
        release_record=V3211_PENDING_RELEASE_RECORD,
        source_commit=source_commit,
        source_bound=True,
        content_overrides={
...
def test_v3226_package_forbids_retired_background_barcode_entrypoints() -> None:
    verifier = load_verifier()
    retired = {
    ...
    assert retired.isdisjoint(verifier.required_files_for_version("3.2.26"))
    assert retired == verifier.forbidden_files_for_version("3.2.26")


def test_v3226_archive_rejects_hidden_reviewed_sop_and_visible_shadow_block(tmp_path: Path) -> None:
    verifier = load_verifier()
    archive_path = tmp_path / "v3226-shadow-sop.zip"
    deploy_path = "docs/sop/06-production-deploy-runbook.md"
    ...
    write_release_archive(
        verifier,
        archive_path,
        content_overrides={deploy_path: deploy},
    )

    with pytest.raises(AssertionError, match="reviewed SOP section differs from approved bytes"):
        verifier.verify_package(archive_path)


def test_v3226_archive_rejects_html_comment_boundaries_around_locked_sop(tmp_path: Path) -> None:
    verifier = load_verifier()
    archive_path = tmp_path / "v3226-comment-boundary-sop.zip"
    deploy_path = "docs/sop/06-production-deploy-runbook.md"
    deploy = (ROOT / deploy_path).read_text(encoding="utf-8")
    deploy = deploy.replace("## Deploy", "<!--\n## Deploy", 1).replace(
    ...
    with pytest.raises(AssertionError, match="HTML comments are forbidden"):
        verifier.verify_package(archive_path)
...
def test_archive_runs_v3211_contract_against_archived_sources(tmp_path: Path) -> None:
    verifier = load_verifier()
    ...
    with pytest.raises(AssertionError, match="V3.2.11 archive source contract"):
        verifier.verify_package(archive_path)


def test_v3211_package_rejects_missing_android_scanner_inputs(tmp_path: Path) -> None:
    verifier = load_verifier()
    archive_path = tmp_path / "missing-android-scanner-inputs.zip"
    write_release_archive(
        verifier,
        archive_path,
        manifest_version="3.2.11",
    ...
    with pytest.raises(AssertionError, match="ConstructionView.vue"):
        verifier.verify_package(archive_path)
...
def test_v3229_build_sequence_executes_current_and_compatible_release_gates_only() -> None:
    build_script = (ROOT / "scripts" / "build-client-release.ps1").read_text(encoding="utf-8")
    ...
    assert "scripts\\verify_v3_2_13_release.py" not in release_verifiers
    assert 'Copy-ReleaseItem "scripts\\verify_v3_2_13_release.py"' in build_script
    assert 'Copy-ReleaseItem "scripts\\test_verify_v3_2_13_release.py"' in build_script
...
def test_v3225_builder_copies_current_release_contract_inputs_into_package() -> None:
    build_script = (ROOT / "scripts" / "build-client-release.ps1").read_text(encoding="utf-8")
...
def test_v3229_builder_executes_complete_contract_regression_and_stops_on_failure(tmp_path: Path) -> None:
    build_script = (ROOT / "scripts" / "build-client-release.ps1").read_text(encoding="utf-8")
    gate_start_marker = 'Write-Host "Running V3.2.29 complete release contract regression..."'
    next_gate_marker = (
        "& .\\.venv\\Scripts\\python.exe -m pytest "
        ".\\v2-api\\tests\\test_collector_transfer_scale.py -q"
    )

    assert gate_start_marker in build_script
    gate_start = build_script.index(gate_start_marker)
    gate_end = build_script.index(next_gate_marker, gate_start)
    gate_block = build_script[gate_start:gate_end]

    invocation_log = tmp_path / "contract-test-arguments.txt"
    fake_python = tmp_path / "failing-python.cmd"
    ...
    harness = tmp_path / "run-v3221-contract-gate.ps1"
    ...
    harness.write_text(
        '\n'.join(
            (
                ...
                f"Set-Content -LiteralPath '{continued}' -Value 'continued'",
            )
        ),
        encoding="utf-8",
    )
    ...
    assert "V3.2.29 complete release contract regression failed" in output
    assert not continued.exists()
    invocation = invocation_log.read_text(encoding="utf-8")
    assert "-m pytest" in invocation
    assert ".\\scripts\\test_verify_v3_2_29_release.py" in invocation
    assert ".\\scripts\\test_verify_client_release.py" in invocation
    assert ".\\scripts\\test_verify_release_sop.py -q" in invocation


def test_current_archive_rejects_an_unapproved_alembic_revision(tmp_path: Path) -> None:
    verifier = load_verifier()
    archive_path = tmp_path / "v3215-unapproved-migration.zip"
    write_release_archive(
        verifier,
        archive_path,
        content_overrides={
            "v2-api/alembic/versions/0017_unapproved_candidate_change.py": (
    ...
    with pytest.raises(AssertionError, match="no new Alembic migration"):
        verifier.verify_package(archive_path)
...
def test_all_copied_operational_documents_reject_stale_candidate_markers() -> None:
    verifier = load_verifier()
    ...
    for document_path in document_paths:
        ...
        verifier.verify_release_markdown_text(document_path, content, "3.2.29")
...
def test_readme_build_command_requires_current_candidate_version() -> None:
    ...
    with pytest.raises(AssertionError, match="non-current release version"):
        verifier.verify_release_markdown_text("README.md", incomplete_command, "3.2.10")
...
def test_archive_source_and_runtime_version_artifacts_must_match(tmp_path: Path) -> None:
    ...
    with pytest.raises(AssertionError, match="V3.2.22 archive source contract|source and built runtime versions"):
        verifier.verify_package(archive_path)
...
def test_archive_rejects_stale_entry_bundle_despite_current_sidecars(tmp_path: Path) -> None:
    ...
    write_release_archive(
        verifier,
        archive_path,
        runtime_version="3.2.29",
        source_version="3.2.29",
        entry_version="3.2.4",
        unrelated_static_version="3.2.29",
        unrelated_chunk_entry_version="3.2.29",
    )
...
@pytest.mark.parametrize(
    ...
    [
        (
            "star-package",
            "Package",
            "* Package: module-manager-v2-server-3.2.29.zip",
        ),
...
def test_archive_rejects_pending_candidate_with_unversioned_claim_and_complete_evidence(
    ...
    record = (
        PENDING_RELEASE_RECORD.replace("- V3.2.22 has not been deployed to production; V3.2.21 remains the immutable rollback target.\n", "")
...
def test_archive_rejects_mutated_v3225_deployed_baseline_record(tmp_path: Path) -> None:
    verifier = load_verifier()
...
def test_archive_accepts_byte_locked_v3213_deployed_baseline_record(tmp_path: Path) -> None:
    verifier = load_verifier()
