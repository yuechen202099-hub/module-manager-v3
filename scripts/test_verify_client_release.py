...
- Release candidate: `V3.2.26`.
- Release-candidate maintenance branch: `production/V3/3.2.26`.
- 当前已部署生产版本：`V3.2.25`。
- 当前发布候选版本：`V3.2.26`。
- 当前候选维护分支：`production/V3/3.2.26`。
"""
PENDING_RELEASE_RECORD = (ROOT / V3226_RELEASE_RECORD).read_text(encoding="utf-8")
...
def write_release_archive(
    ...
    archive_path: Path,
    *,
    manifest_version: str | None = "3.2.26",
    manifest_versions: list[str] | None = None,
    static_version: str = "3.2.26",
    ...
    supported_version = manifest_version in {"3.2.8", "3.2.10", "3.2.11", "3.2.12", "3.2.13", "3.2.14", "3.2.15", "3.2.16", "3.2.17", "3.2.18", "3.2.19", "3.2.20", "3.2.21", "3.2.22", "3.2.23", "3.2.24", "3.2.25", "3.2.26"}
    ...
    contents = {
        ...
        V3226_RELEASE_RECORD: release_record,
    ...
    archive_contents = {
        name: (
            ...
            or (
                name.startswith("v2-api/alembic/versions/")
...
def test_release_builder_default_version_is_candidate_semantic_version() -> None:
    ...
    assert '[string]$Version = "3.2.26"' in build_script
...
def test_release_builder_rejects_non_candidate_version_before_output_mutation(
    ...
    assert "Expected exactly 3.2.26" in output
...
def test_release_builder_embeds_the_current_source_commit() -> None:
    ...
    assert (
        "verify_v3_2_26_release.py --phase package --package $zipPath "
...
def test_release_builder_guards_candidate_version_before_output_mutation() -> None:
    ...
    candidate_guard = build_script.index("Expected exactly 3.2.26")
...
def test_v3226_build_sequence_executes_current_and_compatible_release_gates_only() -> None:
    build_script = (ROOT / "scripts" / "build-client-release.ps1").read_text(encoding="utf-8")
...
def test_v3226_builder_executes_contract_tests_and_stops_on_failure(tmp_path: Path) -> None:
    build_script = (ROOT / "scripts" / "build-client-release.ps1").read_text(encoding="utf-8")
    gate_start_marker = 'Write-Host "Running V3.2.26 contract tests..."'
    next_gate_marker = (
    ...
    assert "V3.2.26 contract tests failed" in output
    assert not continued.exists()
    invocation = invocation_log.read_text(encoding="utf-8")
    assert "-m pytest" in invocation
    assert ".\\scripts\\test_verify_v3_2_26_release.py -q" in invocation
...
def test_all_copied_operational_documents_reject_stale_candidate_markers() -> None:
    ...
    for document_path in document_paths:
        ...
        verifier.verify_release_markdown_text(document_path, content, "3.2.26")
...
def test_archive_rejects_stale_entry_bundle_despite_current_sidecars(tmp_path: Path) -> None:
    ...
    write_release_archive(
        verifier,
        archive_path,
        runtime_version="3.2.26",
        source_version="3.2.26",
        entry_version="3.2.4",
        unrelated_static_version="3.2.26",
        unrelated_chunk_entry_version="3.2.26",
    )
...
@pytest.mark.parametrize(
    ...
    [
        (
            "star-package",
            "Package",
            "* Package: module-manager-v2-server-3.2.26.zip",
        ),
