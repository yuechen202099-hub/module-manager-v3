...
VERSION = "3.2.19"
DISPLAY_VERSION = "V3.2.19"
DEPLOYED_BASELINE = "V3.2.18"
MAINTENANCE_BRANCH = "production/V3/3.2.19"
MIGRATION_REVISION = "20260824_0016"
RELEASE_PATH = "ops/releases/V3.2.19.md"
BASELINE_RELEASE_PATH = "ops/releases/V3.2.18.md"
ARCHIVE_PATH = "build/server-release/module-manager-v2-server-3.2.19.zip"
PRODUCTION_RECORD_SHA256 = "740a63cd4c2dc70b296d2aed106c8cc7097255020bbcc9448d000f64c36c38ad"
OPERATOR_PRODUCTION_MARKERS = (
    "当前生产 release：`/opt/module-manager-v2/releases/v3.2.18-20260829T142448Z`",
    "当前回滚 release：`/opt/module-manager-v2/releases/v3.2.17-20260829T095416Z`",
    "当前生产提交：`0c39ffb1a7e2f59062b41cf35dd2312b1e075f8b`",
)
...
VERSION_SURFACES = {
    "v2-api/app/main.py": 'version="3.2.19"',
    "v2-api/app/services/ops_status.py": 'return "3.2.19"',
    "v2-api/pyproject.toml": 'version = "3.2.19"',
    "v2-api/scripts/verify_v3_1_release.py": 'EXPECTED_VERSION = "3.2.19"',
    "v2-api/tests/test_v3_1_release.py": 'EXPECTED_VERSION = "3.2.19"',
    "v2-web/index.html": "Module Manager V3.2.19",
    "v2-web/package.json": '"version": "3.2.19"',
    "v2-web/src/components/AppLayout.vue": "V3.2.19",
    "v2-web/src/constants/releaseNotes.ts": "version: 'V3.2.19'",
    "v2-web/src/version.json": '"version":"3.2.19"',
    "RELEASE_MANIFEST.md": "- Version: 3.2.19",
}
...
V3217_FEATURE_MARKERS = {
    ...
    "v2-api/app/services/data_center.py": (
        ...
        "def build_meter_module_workbook(",
        'DASHBOARD_IGNORED_EXCEPTION_REASON = "missing_collector_photo"',
    ),
    "v2-api/app/services/state_repository.py": (
        "def resolve_data_center_group_anomaly(",
    ...
    "v2-web/src/api/services.ts": (
        "export async function downloadProjectMeterModuleWorkbook()",
        "'/groups/data-center/export-meter-module'",
    ),
    "v2-web/src/components/data-center/DataCenterGroupReviewPanel.vue": (
        "异常原因",
        "确认已修复",
...
V3217_TEST_MARKERS = (
    (
        "v2-api/tests/test_data_center.py",
        "def test_data_center_meter_module_export_is_current_project_xlsx(",
        "project-scoped meter-module XLSX regression",
    ),
    (
        "v2-api/tests/test_data_center_review.py",
        "def test_json_data_center_anomaly_resolution_keeps_history_and_reopens_after_evidence_change(",
        "anomaly resolution lifecycle regression",
    ),
    (
        "v2-api/tests/test_data_center_review.py",
        "def test_data_center_anomaly_resolution_route_uses_server_actor_and_rejects_stale_evidence(",
        "anomaly audit actor and conflict regression",
    ),
    (
        "v2-api/tests/test_state_repository.py",
        "def test_dual_data_center_anomaly_resolution_rejects_before_json_or_postgres_write(",
        "DualWrite split-brain prevention regression",
    ),
    (
        "v2-web/src/components/__tests__/DataCenterGroupReviewPanel.spec.ts",
        "shows Chinese anomalies and keeps a resolved item as green history",
        "Chinese anomaly history UI regression",
    ),
    (
        "v2-web/src/components/__tests__/ProjectBoardView.spec.ts",
        "downloads the current project meter-module workbook from the cockpit action",
        "dashboard meter-module export UI regression",
    ),
)

V3218_FEATURE_MARKERS = {
    "v2-api/app/services/data_center.py": (
...
V3218_TEST_MARKERS = (
    (
        ...
        "JSON approval anomaly gate regression",
    ),
    (
        "v2-api/tests/test_data_center_review.py",
        "def test_json_approved_review_closes_resolved_exception_state_and_keeps_history(",
        "JSON approved history preservation regression",
    ),
    (
        "v2-api/tests/test_state_repository.py",
        "def test_postgres_incomplete_review_rejects_unconfirmed_anomaly_without_exception_flags(",
        "PostgreSQL incomplete approval bypass regression",
    ),
    (
        "v2-web/src/components/__tests__/DataCenterGroupReviewPanel.spec.ts",
        "separates current anomalies from resolved history in the review panel",
        "review panel anomaly history regression",
    ),
)

V3219_METER_DEDUP_MARKERS = {
    ...
    "v2-api/app/services/collector_transfer.py": (
        ...
        "meter_projection.source or source_by_group[internal_group_id]",
    ),
    "v2-api/tests/test_collector_transfer_domain.py": (
        "def test_terminal_snapshot_deduplicates_collectors_and_duplicate_meter_rows()",
    ),
    "v2-api/tests/test_collector_transfer_service.py": (
        "def test_review_workbench_deduplicates_same_meter_and_merges_device_numbers(",
    ),
    "v2-api/tests/test_terminal_review_domain.py": (
        "def test_same_meter_merges_constructed_and_unconstructed_identity_without_blocking_rephoto()",
        "def test_unconstructed_same_meter_cannot_override_constructed_device_numbers()",
    ),
}
    ...
    *,
    ...
    for relative_path in REQUIRED_FILES:
        if not (root / relative_path).is_file():
            failures.append(f"{relative_path}: required V3.2.19 file is missing")
    ...
    for path, markers in FEATURE_MARKERS.items():
        label = "V3.2.19 rephoto and preview contract"
    ...
    for path, markers in TASK_FEATURE_MARKERS.items():
        _require_markers(root, path, markers, "V3.2.19 Task 1-4 feature contract", failures)
    for path, marker, label in V3213_SCANNER_MARKERS:
        _require_markers(root, path, (marker,), label, failures)
    for path, marker, label in TASK_TEST_MARKERS:
        _require_markers(root, path, (marker,), label, failures)
    for path, markers in V3216_FEATURE_MARKERS.items():
        _require_markers(root, path, markers, "V3.2.19 feature contract", failures)
    for path, marker, label in V3216_TEST_MARKERS:
        _require_markers(root, path, (marker,), label, failures)
    for path, marker, label in V3216_RELEASE_TEST_MARKERS:
        _require_markers(root, path, (marker,), label, failures)
    for path, markers in V3217_FEATURE_MARKERS.items():
        _require_markers(root, path, markers, "V3.2.19 anomaly/export feature contract", failures)
    ...
    for path, marker, label in V3217_TEST_MARKERS:
        _require_markers(root, path, (marker,), label, failures)
    for path, markers in V3218_FEATURE_MARKERS.items():
        _require_markers(root, path, markers, "V3.2.19 approved-exception closure", failures)
    for path, marker, label in V3218_TEST_MARKERS:
        _require_markers(root, path, (marker,), label, failures)
    for path, markers in V3219_METER_DEDUP_MARKERS.items():
        _require_markers(root, path, markers, "V3.2.19 meter deduplication contract", failures)

    release_tool_markers = {
        "scripts/build-client-release.ps1": (
            '[string]$Version = "3.2.19"',
            MAINTENANCE_BRANCH,
            "scripts\\verify_v3_2_16_release.py",
            ...
            "verify_v3_2_19_release.py --phase package --package $zipPath --expected-source-commit $sourceCommit",
        ),
        "scripts/verify-client-release.py": (
            '"scripts/verify_v3_2_16_release.py"',
            '"scripts/test_verify_v3_2_16_release.py"',
            ...
            '"ops/releases/V3.2.18.md"',
            '"ops/releases/V3.2.19.md"',
            "verify_v3219_archive_source_contract",
            "verify_v3218_archive_source_contract",
            "verify_v3217_archive_source_contract",
            "verify_v3216_archive_source_contract",
            "verify_v3214_archive_source_contract",
        ),
        "scripts/verify_release_sop.py": (
            'with_name("verify_v3_2_19_release.py")',
            'candidate == "V3.2.19"',
            '"scripts/verify_v3_2_19_release.py"',
            '"scripts/verify_v3_2_18_release.py"',
            '"scripts/verify_v3_2_17_release.py"',
    ...
    for path, markers in release_tool_markers.items():
        _require_markers(root, path, markers, "V3.2.19 release gate", failures)
...
def _load_generic_package_verifier(root: Path):
    ...
    spec = importlib.util.spec_from_file_location("v3217_generic_package_verifier", path)
...
def _check_package(
    ...
    if expected_source_commit is None:
        failures.append("package phase requires --expected-source-commit for V3.2.19")
        return
    ...
    if re.fullmatch(r"[0-9a-f]{40}", normalized_expected_commit) is None:
        failures.append("V3.2.19 expected source commit must be one lowercase 40-character Git commit")
        return
    if package_path is None:
        failures.append("package phase requires --package for V3.2.19")
        return
    package = Path(package_path)
    if not package.is_file() or package.stat().st_size <= 0:
        failures.append(f"V3.2.19 package is missing or empty: {package}")
        return
    try:
        with ZipFile(package) as archive:
            ...
            if "SOURCE_COMMIT" not in name_set:
                failures.append("V3.2.19 package requires SOURCE_COMMIT")
            else:
                source_commit = archive.read("SOURCE_COMMIT").decode("ascii", errors="replace").strip()
                if re.fullmatch(r"[0-9a-f]{40}", source_commit) is None:
                    failures.append("V3.2.19 SOURCE_COMMIT must be one lowercase 40-character Git commit")
            if "RELEASE_MANIFEST.md" not in name_set:
                failures.append("V3.2.19 package requires RELEASE_MANIFEST.md")
            else:
                manifest_bytes = archive.read("RELEASE_MANIFEST.md")
                if b"\r" in manifest_bytes:
                    failures.append("V3.2.19 RELEASE_MANIFEST.md must use LF line endings")
                manifest = manifest_bytes.decode("utf-8", errors="replace")
                versions = re.findall(r"(?m)^- Version:\s*(\S+)\s*$", manifest)
                if versions != [VERSION]:
                    failures.append(f"V3.2.19 package manifest must contain exactly Version {VERSION}")
    except (BadZipFile, OSError) as exc:
        failures.append(f"V3.2.19 package cannot be opened: {exc}")
        return
    ...
    try:
        ...
        failures.append(f"V3.2.19 package verification failed: {exc}")
...
def _check_attestation(
    ...
    path_patterns = {
        "Backup directory": r"/opt/module-manager-v2/backups/v3\.2\.19-\d{8}T\d{6}Z",
        "Release directory": r"/opt/module-manager-v2/releases/v3\.2\.19-\d{8}T\d{6}Z",
        "Rollback directory": r"/opt/module-manager-v2/releases/v3\.2\.18-\d{8}T\d{6}Z",
    }
    ...
    for field in ("Local health", "Public health"):
        ...
        if (
            len(values) != 1
            or re.fullmatch(r"HTTP\s+200\s+version\s+3\.2\.19", values[0], re.IGNORECASE) is None
        ):
            failures.append(f"{RELEASE_PATH}: {field} must prove HTTP 200 for version 3.2.19")
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Verify the V3.2.19 source, package, or attestation contract.")
    ...
    if failures:
        ...
        return 1
    print(f"[OK] V3.2.19 {args.phase} release contract")
    return 0
