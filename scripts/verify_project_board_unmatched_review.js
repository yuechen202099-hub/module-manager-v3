...
assertContains(unmatchedRegionScan, 'regionScanDialogActive = true', 'recognized candidates must activate one confirmation cycle')
...
assertContains(unmatchedRegionReplace, 'closeRegionScanDialog()', 'replacement must close through the serial-invalidating path')
...
assertContains(dialog, 'let regionScanDialogActive = false', 'unmatched confirmation needs an active-cycle guard')
const unmatchedRegionClose = section(dialog, 'function closeRegionScanDialog()', 'function invalidateRegionScan')
assertContains(unmatchedRegionClose, 'if (!regionScanDialogActive) return', 'duplicate closed events must be side-effect free')
assertContains(unmatchedRegionClose, 'regionScanDialogActive = false', 'closing must consume the active confirmation cycle')
assertContains(unmatchedRegionClose, 'regionScanSerial += 1', 'closing must invalidate the captured scan serial')
const unmatchedRegionInvalidate = section(dialog, 'function invalidateRegionScan()', 'function openImagePreview')
assertContains(unmatchedRegionInvalidate, 'regionScanDialogActive = false', 'context invalidation must consume the active confirmation cycle')
assertNotContains(unmatchedRegionInvalidate, 'closeRegionScanDialog()', 'context invalidation must not trigger duplicate close side effects')
assertContains(dialog, '@click="closeRegionScanDialog">取消', 'cancel must use the serial-invalidating close path')
assertContains(dialog, '@closed="closeRegionScanDialog"', 'system close must use the serial-invalidating close path')
